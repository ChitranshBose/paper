...
with tf.gfile.FastGFile("./model/atm1-model.pb", 'rb') as f:
...
classes = [line.rstrip() for line
                   in tf.gfile.GFile("./model/atm1-model.csv")]

with tf.Session() as sess:
    
    saver = tf.train.import_meta_graph('./model/atm1-model.meta')
    saver.restore(sess, "./model/atm1-model")
    
    y_pred = sess.graph.get_tensor_by_name('y_pred:0')
    x = sess.graph.get_tensor_by_name("x:0")
      
    path = 'testing_data_ATM'
    files = sorted(os.listdir(path))
    
    for filename in files:
            
        with open('csv_ATM/' + filename + '.csv', 'ab') as csvfile:
